use std::error::Error;
use stripe::{Client, CreatePaymentIntent, ErrorType, PaymentIntent, PaymentIntentConfirmParams, PaymentIntentId, StripeError};
...
impl From<stripe::PaymentIntentStatus> for PaymentStatus {
    fn from(status: stripe::PaymentIntentStatus) -> Self {
        match status {
            ...
            stripe::PaymentIntentStatus::RequiresAction => PaymentStatus::RequiresAction(status.to_string()),
            stripe::PaymentIntentStatus::Canceled => PaymentStatus::Failed,
            stripe::PaymentIntentStatus::Processing => PaymentStatus::Pending,
            stripe::PaymentIntentStatus::RequiresCapture => PaymentStatus::RequiresAction(status.to_string()),
            stripe::PaymentIntentStatus::RequiresConfirmation => PaymentStatus::RequiresAction(status.to_string()),
            stripe::PaymentIntentStatus::RequiresPaymentMethod => PaymentStatus::RequiresAction(status.to_string()),
        }
    }
}
...
impl PaymentService for StripePaymentService {
    ...
    async fn create_payment_intent(&self, amount: i64, currency: &str) -> Result<Self::PaymentIntent, PaymentError> {
        let currency = stripe::Currency::from_str(currency)
            .map_err(|_| PaymentError::InvalidCurrency)?;

        let payment_intent = stripe::CreatePaymentIntent::new(
            amount,
            currency,
        );
    ...
    async fn confirm_payment_intent(&self, payment_intent_id: &str) -> Result<Self::PaymentIntent, PaymentError> {
        ...
        let payment_intent = stripe::PaymentIntent::confirm(
            &self.client,
            &payment_intent_id,
            payment_params
        ).await;
    ...
    async fn check_payment_status(&self, payment_intent_id: &str) -> Result<Self::Status, PaymentError> {
        ...
        let status = stripe::PaymentIntent::retrieve(&self.client, &payment_id, &[]).await
    ...
    async fn charge_payment(&self, payment_intent_id: &str) -> Result<Self::PaymentIntent, PaymentError> {
        ...
        let payment_amount =
            PaymentIntent::retrieve(&self.client, &safe_payment_intent_id, &[]).await
        ...
        PaymentIntent::capture(&self.client, &safe_payment_intent_id, capture_params).await
            .map_err(|err| PaymentError::from(err))
    }
}
...
mod tests {
    ...
    async fn test_create_payment_intent() {
        dotenv::dotenv().ok();
        let stripe_secret = std::env::var("STRIPE_TEST_SECRET_KEY").expect("STRIPE_TEST_SECRET_KEY must be set");
    ...
    async fn test_confirm_payment_intent() {
        ...
        assert_eq!(confirmed_intent.status, stripe::PaymentIntentStatus::RequiresPaymentMethod);
    }
    ...
    async fn test_check_payment_status() {
        ...
        let confirmed_intent = service.confirm_payment_intent(&intent.id).await.unwrap();
        let status = service.check_payment_status(&confirmed_intent.id).await.unwrap();
